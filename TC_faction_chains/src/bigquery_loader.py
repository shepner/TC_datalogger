...
class BigQueryLoader:
    ...
    def _build_merge_statement(
        ...
        # Build UPDATE SET clause (quote field names to handle reserved keywords)
        update_set = ", ".join([f"target.`{f}` = source.`{f}`" for f in update_fields])

        # Build INSERT VALUES clause (quote field names)
        insert_values = ", ".join([f"source.`{f}`" for f in insert_fields])

        # Use direct comparison for id field (now INTEGER type) - quote field name
        key_compare = f"target.`{key_field}` = source.`{key_field}`"
        
        # Quote field names in INSERT clause
        quoted_insert_fields = ", ".join([f"`{f}`" for f in insert_fields])

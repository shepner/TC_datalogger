# Torn City Items to BigQuery

A data pipeline that automatically fetches Torn City items catalog from the Torn City API and stores it in Google BigQuery. The pipeline runs every 15 minutes within a Docker container.

## Overview

This project implements a robust ETL pipeline that:
- Fetches items catalog from Torn City API v2 (`/v2/torn/items`)
...
1. **Build the Docker image**:
   ```bash
   docker build -t tc-items .
   ```

2. **Run the container**:
   ```bash
   docker run -d \
     --name tc-items-pipeline \
...
3. **View logs**:
   ```bash
   docker logs -f tc-items-pipeline
   ```
...
4. **Run specific endpoint**:
   ```bash
   python -m src.main --endpoint v2_torn_items
   ```
...
- Useful for reference data that changes over time (like items catalog)
...
- Torn City API key with items access
...
### View Container Logs
```bash
docker logs tc-items-pipeline
```

### Check Cron Logs
```bash
docker exec tc-items-pipeline cat /app/logs/cron.log
```

### Run Manually Inside Container
```bash
docker exec -it tc-items-pipeline python -m src.main
```

# Torn City Faction Members to BigQuery

A data pipeline that automatically fetches faction member data from the Torn City API and stores it in Google BigQuery. The pipeline runs every 15 minutes within a Docker container.

## Overview

This project implements a robust ETL pipeline that:
- Fetches faction member records from Torn City API v2 (`/v2/faction/members`)
- Handles pagination to retrieve all available records
- Stores records in Google BigQuery using **replace mode** (full table replacement each run)
...
1. **Build the Docker image**:
   ```bash
   docker build -t tc-faction-members .
   ```

2. **Run the container**:
   ```bash
   docker run -d \
     --name tc-faction-members-pipeline \
...
3. **View logs**:
   ```bash
   docker logs -f tc-faction-members-pipeline
   ```
...
4. **Run specific endpoint**:
   ```bash
   python -m src.main --endpoint v2_faction_40832_members
   ```
...
- **Storage Mode**: `replace` (full table replacement each run)
- **Frequency**: `PT15M` (15 minutes)

### Storage Mode: Replace

This service uses **replace mode**, which means:
- Each run completely replaces the entire BigQuery table
- No deduplication is needed (table is wiped and reloaded)
- Useful for reference data that changes over time (like member lists)
...
- ✅ **Replace Mode**: Full table replacement ensures data is always current
...
- Torn City API key with faction members access
...
4. **Data Loading**: 
   - **Replace mode**: Overwrites entire table with new data (WRITE_TRUNCATE)
...
### View Container Logs
```bash
docker logs tc-faction-members-pipeline
```

### Check Cron Logs
```bash
docker exec tc-faction-members-pipeline cat /app/logs/cron.log
```

### Run Manually Inside Container
```bash
docker exec -it tc-faction-members-pipeline python -m src.main
```
